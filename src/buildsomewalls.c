...
uint32_t    paint_wall_slice(t_player *player, t_vars *game)
{
    ...
    i = y_coordinate * TEXTURE_W + x_coordinate * game->texture->bytes_per_pixel;
...
void    texture_coordinates(t_vars *game)
{
    ...
    game->player->x_texture = game->x_wall/2 * (double)game->texture->width - 1; // -1)
    // if ((game->player->ray->x_ray_dir > 0 && game->player->ray->side == 0) || 
    //     (game->player->ray->y_ray_dir < 0 && game->player->ray->side == 1))
    //     game->player->x_texture = game->texture->width - game->player->x_texture;
...
void    render_wall_slice(int x, t_player *player, t_vars *game)
{
    ...
    {
        player->y_texture = (int)position_of_texture;
        if (player->y_texture >= TEXTURE_H)
            player->y_texture = TEXTURE_H - 1; //do not delete it
